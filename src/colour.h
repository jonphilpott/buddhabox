...
float JUST_OFFSETS[] = {1.0f, 1.067f, 1.125f, 1.2f, 1.25f,  1.33f, 1.406f,
                        1.5f, 1.6f,   1.667f, 1.8f, 1.875f, 2.0f};
...
public:
  ...
  void setRoot(float r) { root_ = r; }

  float getRoot() { return root_; }

  void newMelody(LFSR *rng, uint8_t rmax) {
    melody_[0] = 0;

    for (int i = 0; i < (MELODY_LENGTH - 1); i++) {
      int n = rng->nextRange(0, rmax);

      // naive best effort to encourage melodies without repeated notes
      if (i > 0) {
        if (melody_[i - 1] == n) {
          n = rng->nextRange(0, rmax);
        }
      }
  ...
  uint8_t getMelody(uint8_t idx) { return melody_[idx % MELODY_LENGTH]; }

private:
  uint8_t colour_ = 0;
  float offsets_[5] = {JUST_OFFSETS[0], JUST_OFFSETS[1], JUST_OFFSETS[5],
                       JUST_OFFSETS[7], JUST_OFFSETS[8]};
